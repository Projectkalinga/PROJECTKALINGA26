...
export function ComponentCard({ data, index }: ComponentCardProps) {
    return (
        <motion.div
            ...
            className="group relative bg-[#121413] border border-white/10 rounded-xl overflow-hidden hover:border-kalinga/50 transition-colors duration-300 flex flex-col h-full"
        >
            {/* Top Image Section */}
            <div className="relative w-full h-48 bg-black/50 overflow-hidden border-b border-white/5">
            ...
            <div className="p-5 flex-1 flex flex-col">
                ...
                <div className="mb-4">
                    ...
                    <h3 className="text-lg font-heading text-white leading-tight">{data.name}</h3>
                </div>

                {/* Role Description */}
                <p className="text-sm text-gray-400 font-mono leading-relaxed mb-6 border-l-2 border-white/10 pl-3 group-hover:border-kalinga/50 transition-colors">
                    {data.description}
                </p>

                {/* Specs Table */}
                <div className="mt-auto bg-black/30 rounded border border-white/5 p-3">
                    <table className="w-full text-left border-collapse">
                        <tbody>
                            {Object.entries(data.specs).map(([key, value]) => (
                                <tr key={key} className="border-b border-white/5 last:border-0">
                                    <td className="py-1 text-[10px] font-mono text-gray-500 uppercase">{key}</td>
                                    <td className="py-1 text-[10px] font-mono text-kalinga text-right">{value}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
